...
import {
  ...
  fetchAcademicYears,
  updateAcademicYear,
} from '@/redux/slice/academic-year.slice';
import type { AcademicYear } from '@/types/api.types';

export const useAcademicYear = () => {
  const dispatch = useAppDispatch();
  const { academicYears, academicYear, loading, error } = useAppSelector(
  ...
  const handleFetchAcademicYear = useCallback(
    (id: number) => {
  ...
  const handleUpdateAcademicYear = useCallback(
    (id: number, academicYear: AcademicYear) => {
