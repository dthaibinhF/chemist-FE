...
export const AddStudentCsvFile = ({
  ...
  const hasErrors = useMemo(() => {
    ...
    return studentsPreview.some((student) =>
      Object.values(student.errors).some((error) => error !== null)
    );
  }, [studentsPreview]);
      ...
      Papa.parse(file, {
        ...
        complete: (results) => {
          ...
          if (results.errors.length > 0) {
            toast.error(
              "Có lỗi xảy ra khi đọc tệp. Vui lòng kiểm tra định dạng tệp CSV."
            );
          ...
          // Convert selected IDs to display names for defaults
          const selectedSchoolName = selectedSchool
            ? schools.find((s) => s.id?.toString() === selectedSchool)?.name ||
              selectedSchool
            : "";
          const selectedGradeName = selectedGrade
            ? grades.find((g) => g.id?.toString() === selectedGrade)?.name ||
              selectedGrade
            : "";
          const selectedSchoolClassName = selectedSchoolClass
            ? schoolClasses.find(
                (sc) => sc.id?.toString() === selectedSchoolClass
              )?.name || selectedSchoolClass
            : "";
          const selectedAcademicYearName = selectedAcademicYear
            ? academicYears.find(
                (ay) => ay.id?.toString() === selectedAcademicYear
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      const groupValidationResults: Array<{
      ...
      for (const student of studentsPreview) {
        if (student.data.group_name) {
          const validation =
            groupValidationMap.get(student.data.group_name) || {};
      ...
      const groupErrors = groupValidationResults.filter(
        (result) => result.error
      );
      ...
      const studentsToSave = studentsPreview.map((s, index) => {
        ...
        const schoolClass = s.data.school_class
          ? schoolClasses.find(
              (schoolClassItem) => schoolClassItem.name === s.data.school_class
            )
          : undefined;

        const academicYear = s.data.academic_year
          ? academicYears.find(
              (academicYearItem) => academicYearItem.year === s.data.academic_year
            )
          : undefined;
      ...
      toast.promise(studentService.createMultipleStudents(studentsToSave), {
  ...
  const debouncedValidation = useCallback(
    debounce((rowIndex: number, defaults: any) => {
      setStudentsPreview((currentPreview) => {
        ...
        const revalidatedStudent = validateStudentRow(
          newPreview[rowIndex].data,
          rowIndex,
          defaults
        );
    ...
    <div className="space-y-4 h-full flex flex-col">
      <Card>
        <CardHeader>
          ...
          <CardDescription>
            Thiết lập giá trị mặc định cho các trường không bắt buộc. Giá trị
            này sẽ được áp dụng khi cột tương ứng trong CSV để trống.
          </CardDescription>
        </CardHeader>
        <CardContent className="grid grid-cols-1 gap-4 md:grid-cols-2 lg:grid-cols-5">
          ...
          <SchoolClassSelect
            ...
            handleSelect={setSelectedSchoolClass}
          />
          <AcademicYearSelect
            value={selectedAcademicYear}
            handleSelect={setSelectedAcademicYear}
          />
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>
            Tải lên file CSV đã điền theo template. Hệ thống sẽ kiểm tra định
            dạng và hiển thị dữ liệu để xác nhận.
          </CardDescription>
        </CardHeader>
        <CardContent className="space-y-4">
          <div className="rounded-md bg-blue-50 p-4">
            <h4 className="font-medium text-blue-900 mb-2">
              Cấu trúc CSV Template cần sử dụng:
            </h4>
          ...
          <div className="rounded-md bg-green-50 p-4">
            ...
            <div className="text-sm text-green-800">
              ...
              <p className="font-medium ml-4 text-red-700">
                • Cột "group_name": Nhập tên nhóm CHÍNH XÁC (VD: "12NC1",
                "11VL2") - không được sai một ký tự
              </p>
              <p className="font-medium ml-4">
                • Cột "school": Có thể dùng viết tắt (VD: AK, CVL) hoặc tên đầy
                đủ
              </p>
          ...
          <div className="rounded-md bg-red-50 p-4 mb-4">
            <h4 className="font-medium text-red-900 mb-2">
              ⚠️ Lưu ý quan trọng về tên nhóm:
            </h4>
            <div className="text-sm text-red-800 space-y-1">
              <p>
                <strong>Tên nhóm phải chính xác 100%</strong> - hệ thống sẽ từ
                chối nếu không khớp hoàn toàn
              </p>
              <p>• Ví dụ đúng: "12NC1", "11VL2", "10TK3"</p>
              <p>
                • Ví dụ sai: "12NC" (thiếu số), "12nc1" (sai viết hoa), "12NC 1"
                (có khoảng trắng)
              </p>
              <p className="font-medium">
                Kiểm tra kỹ tên nhóm trước khi import!
              </p>
            </div>
          </div>
          ...
          <div className="rounded-md bg-yellow-50 p-4">
            <h4 className="font-medium text-yellow-900 mb-2">
              Các viết tắt tên trường hỗ trợ:
            </h4>
            <div className="text-sm text-yellow-800 grid grid-cols-2 gap-1">
              <p>
                <strong>AK</strong> → Trường THPT An Khánh
              </p>
              <p>
                <strong>CVL</strong> → Trường THPT Châu Văn Liêm
              </p>
              <p>
                <strong>NVD</strong> → Trường THPT Nguyễn Việt Dũng
              </p>
              <p>
                <strong>NVH</strong> → Trường THPT Nguyễn Việt Hồng
              </p>
              <p>
                <strong>PNH</strong> → Trường THPT Phan Ngọc Hiển
              </p>
              <p>
                <strong>BHN</strong> → Trường THPT Bùi Hữu Nghĩa
              </p>
              <p>
                <strong>BM</strong> → Trường THPT Bình Minh
              </p>
              <p>
                <strong>LTT</strong> → Trường THPT Lý Tự Trọng
              </p>
              <p>
                <strong>LHP</strong> → Trường THPT Lưu Hữu Phước
              </p>
              <p>
                <strong>NS</strong> → Trường THPT Ngã Sáu
              </p>
              <p>
                <strong>THSP</strong> → Trường THPT Thực Hành Sư Phạm
              </p>
              <p>
                <strong>TSTD</strong> → Thí Sinh Tự Do
              </p>
            </div>
              ...
              <FormField
                ...
                render={({ field }) => (
                  <FormItem className="flex-grow">
                    <FormLabel>
                      Chọn file CSV (phải theo đúng template)
                    </FormLabel>
                    <FormControl>
                      <Input
                        type="file"
