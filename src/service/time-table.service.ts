...
export const generateNextWeekSchedules = async (): Promise<string> => {
  const response = await apiClient.post("/bulk/next-week");
  return response.data;
}
export const triggerAutoGeneration = async (): Promise<string> => {
