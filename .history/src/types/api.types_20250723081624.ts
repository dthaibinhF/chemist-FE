export interface BaseDTO {
  ...
  /**
   * Support both old (create_at/update_at) and new (created_at/updated_at) timestamp
   * field names so that we stay backward-compatible while aligning with the
...
export interface Schedule extends BaseDTO {
  group_id: number;
  /**
   * The BE does not guarantee to include `group_name` in every response, so make
   * this field optional.
   */
  group_name?: string;
  start_time: Date;
  end_time: Date;
  /**
   * New identifiers returned by the Schedule endpoints
   */
  room_id?: number;
  teacher_id?: number;
